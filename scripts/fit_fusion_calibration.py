...
def load(path: Path):
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        row = json.loads(line)
        cands = row.get("features", {}).get("candidates", [])
...
def main():
    ...
    params = fit(train)
    evaluate(params, train, "train")
    evaluate(params, hold, "holdout")
    a, b, w = params[0:3], params[3:6], np.log1p(np.exp(params[6:9]))
    print("\nRust constants (paste into the CALSUM mode):")
    print(f"  // fitted {PATH.name}: per-leg logistic calibration + weights")
    for name, i in (("VEC", 0), ("BM25", 1), ("GRAPH", 2)):
        print(
            f"  const CALSUM_{name}: (f32, f32, f32) = "
            f"({a[i]:.4f}, {b[i]:.4f}, {w[i]:.4f}); // (intercept, slope, weight)"
        )
